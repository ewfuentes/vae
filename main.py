...
def main(num_epochs: int, batch_size: int, learning_rate: float, kl_factor: float, latent_dim: int):
    # Build a dataset
    ...
    # Build the model
    autoencoder = VariationalAutoEncoder(channel_dims=[8, 16, 32], latent_dim=latent_dim).cuda()
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx, (imgs, _) in enumerate(train_loader):
            ...
            loss.backward()
            opt.step()
            print(f"{epoch_idx=} {batch_idx=} loss={loss.detach().item():0.03f} {kl_loss=:0.3f} {reconstruction_loss=:0.3f}", end='\r')
            writer.add_scalar("train/kl_loss", kl_loss.detach().item(), global_step=total_batch_idx)
            writer.add_scalar("train/reconstruction_loss", reconstruction_loss.detach().item(), global_step=total_batch_idx)
        print()
        num_items = 0
        kl_loss = 0.0
        kl_sum = 0.0
        ...
        with torch.no_grad():
            for (imgs, _) in test_loader:
                imgs = imgs.cuda()
                mu, logvar, _, x_prime_logit = autoencoder(imgs)
                raw_kl = compute_reverse_kl_loss(mu, logvar, should_reduce=False).mean(dim=(0)).flatten()
        ...
        writer.add_scalar("val/reconstruction_loss", reconstruction_loss.item(), global_step=epoch_idx)
        ...
        writer.add_histogram_raw("val/kl_per_dim", 
                                 min=kl_bins[0],
                                 max=kl_bins[-1],
                                 num=kl_counts.sum(),
                                 sum=kl_sum,
...
if __name__ == "__main__":
    ...
    main(args.num_epochs, args.batch_size, args.learning_rate, args.kl_factor, args.latent_dim)
